...
class SwitBinding extends Bindings {
  ...
  void dependencies() {
    // DataSource
    Get.lazyPut<StudyGroupRemoteDataSource>(
          () => StudyGroupRemoteDataSource(),
    );

    // Repository
    Get.lazyPut<StudyGroupRepository>(
          () => StudyGroupRepositoryImpl(Get.find<StudyGroupRemoteDataSource>()),
    );

    // UseCases
    Get.lazyPut(() => CreateStudyGroupUseCase(Get.find<StudyGroupRepository>()));
    Get.lazyPut(() => InviteToGroupUseCase(Get.find<StudyGroupRepository>()));

    // ViewModel
    Get.lazyPut(() => SwitViewModel(
      createStudyGroupUseCase: Get.find<CreateStudyGroupUseCase>(),
      inviteToGroupUseCase: Get.find<InviteToGroupUseCase>(),
    ));
  }
}